...
import com.putao.widget.pulltorefresh.internal.FlipLoadingLayout;
import com.putao.widget.pulltorefresh.internal.LoadingLayout;
import com.putao.widget.pulltorefresh.internal.RotateLoadingLayout;
import com.putao.widget.pulltorefresh.internal.Utils;
import com.putao.widget.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    //add end fields
    protected boolean didReachTheEnd;
    protected boolean didReachTheStart;
    ...
    public void setDidReachStatus(boolean didReachTheStart, boolean didReachTheEnd) {
        this.didReachTheStart = didReachTheStart;
        this.didReachTheEnd = didReachTheEnd;
        if (didReachTheStart == true && didReachTheEnd == true) {
            setMode(Mode.DISABLED);
        } else if (didReachTheStart == true && didReachTheEnd == false) {
            setMode(Mode.PULL_FROM_END);
        } else if (didReachTheStart == false && didReachTheEnd == true) {
            setMode(Mode.PULL_FROM_START);
        } else if (didReachTheStart == false && didReachTheEnd == false) {
            setMode(Mode.BOTH);
        }
        //            setMode(didReachTheEnd ? Mode.PULL_FROM_START : Mode.BOTH);
    }
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                if (getScrollY() == 0 && event.getY() - yLast <= 0.0F) {
                    isDownScroll = true;
                } else {
                    isDownScroll = false;
                }
